...
rem Vinext stores its development-server PID in this file.  A stopped or
rem unresponsive previous run can leave the lock behind and make a new run
rem immediately exit without opening the UI.  Only recover a lock that names
rem this exact project after the health check above has failed.
if exist ".vinext\dev\lock.json" (
  set "VINEXT_PID="
  for /f "usebackq delims=" %%I in (`powershell.exe -NoProfile -Command "$lock = Get-Content -Raw '.vinext\dev\lock.json' | ConvertFrom-Json; if ($lock.cwd -eq (Get-Location).Path -and [string]$lock.pid -match '^[0-9]+$') { $lock.pid }" 2^>nul`) do set "VINEXT_PID=%%I"
  if defined VINEXT_PID (
    echo [INFO] Recovering an unresponsive previous UI session...
    powershell.exe -NoProfile -Command "Stop-Process -Id %VINEXT_PID% -Force -ErrorAction SilentlyContinue" >nul 2>nul
    powershell.exe -NoProfile -Command "Start-Sleep -Seconds 1" >nul 2>nul
  )
  rem This directory is generated by Vinext. Rebuilding it prevents a stale
  rem optimizer cache from causing the next UI startup to fail as well.
  rmdir /s /q ".vinext" >nul 2>nul
)
